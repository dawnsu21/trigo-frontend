...
export default function AdminDashboard() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      setError(err?.message || err?.data?.message || 'Unable to load dashboard stats')
    }
  }, [token])
  ...
  const handleDriverStatus = async (driverId, status) => {
    ...
    try {
      ...
      await loadDrivers()
      setSuccess(`Driver ${status === 'approved' ? 'approved' : 'rejected'} successfully!`)
      setTimeout(() => setSuccess(''), 3000)
    } catch (err) {
      setError(err?.data?.message || err?.message || 'Unable to update driver status. Please try again.')
    } finally {
      setBusy(false)
    }
  }
  ...
  if (!token) {
    return (
      <div className="dashboard">
        <div className="alert alert--error">Not authenticated. Please log in.</div>
      </div>
    )
  }
    ...
    <div className="dashboard">
      ...
      <section className="dashboard__grid">
        <article className="panel">
          ...
          <p className="panel__description">Overview of your TriGo platform activity</p>
          <dl className="details">
            ...
            <div>
              ...
              <dd>₱{stats?.today_revenue ? parseFloat(stats.today_revenue).toFixed(2) : '0.00'}</dd>
            </div>
          </dl>
        </article>
        ...
        <article className="panel">
          <div className="panel__header">
            <div>
              ...
              <p className="panel__description">Review and approve new driver applications</p>
            </div>
            <button className="secondary" onClick={loadDrivers}>Refresh List</button>
          </div>
          {drivers.length === 0 && (
            ...
            <div className="queue__item" key={driver.id}>
              <div>
                ...
                <div className="details" style={{ marginTop: '0.5rem' }}>
                  <div>
                    <dt>Email Address</dt>
                    <dd>{driver.email || driver.user?.email || 'Not provided'}</dd>
                  </div>
                  <div>
                    <dt>Phone Number</dt>
                    <dd>{driver.phone || driver.user?.phone || 'Not provided'}</dd>
                  </div>
                  <div>
                    <dt>Vehicle Information</dt>
                    <dd>{driver.vehicle_type || (driver.vehicle_make && driver.vehicle_model ? `${driver.vehicle_make} ${driver.vehicle_model}` : 'Not provided')}</dd>
                  </div>
                  <div>
                    <dt>License Plate Number</dt>
                    <dd>{driver.plate_number || driver.license_plate || 'Not provided'}</dd>
                  </div>
                  <div>
                    <dt>Application Status</dt>
                    <dd>
                      <span className={`status status--${driver.status?.toLowerCase()}`}>
                        {driver.status === 'pending' && 'Pending Review'}
                        {driver.status === 'approved' && 'Approved'}
                        {driver.status === 'rejected' && 'Rejected'}
                        {!['pending', 'approved', 'rejected'].includes(driver.status) && driver.status}
                      </span>
                    </dd>
                  </div>
                </div>
      ...
      <section className="panel">
        <div className="panel__header">
          <div>
            ...
            <p className="panel__description">Track all rides across the platform</p>
          </div>
          <button className="secondary" onClick={loadRides}>Refresh Data</button>
        </div>
        {rides.length === 0 && (
          <div className="panel__empty">
            <p>No rides have been booked yet.</p>
            <p className="text-muted">Ride information will appear here as passengers book rides.</p>
          </div>
        )}
        {rides.length > 0 && (
          <div className="table">
              ...
              <div className="table__row" key={ride.id}>
                  ...
                  {ride.passenger ? (
                    <>
                      ...
                      {ride.passenger.email && (
                        <>
                          <br />
                          <small className="text-muted">{ride.passenger.email}</small>
                        </>
                      )}
                    </>
                  ) : (
                ...
                <span>
                  <div style={{ fontSize: '0.875rem' }}>
                    <strong>From:</strong> {ride.pickup_place?.name || ride.pickup_address || 
                      (ride.pickup_lat && ride.pickup_lng ? 'Coordinates' : 'N/A')}
                  </div>
                ...
                <span>
                  <span className={`status status--${ride.status?.toLowerCase().replace('_', '-')}`}>
                    ...
                    {!['pending', 'accepted', 'picked_up', 'completed', 'cancelled'].includes(ride.status) && ride.status}
                  </span>
                </span>
