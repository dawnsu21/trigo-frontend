...
export default function DriverDashboard() {
  ...
  const loadQueue = useCallback(async () => {
    try {
      ...
      setError(err?.message || err?.data?.message || 'Unable to load ride queue')
    }
  }, [token])
  ...
  const toggleAvailability = async () => {
    ...
    try {
      ...
      setSuccess(newStatus ? 'You are now online and will receive ride requests!' : 'You are now offline.')
      ...
      setError(err?.data?.message || err?.message || 'Unable to update status. Please try again.')
    } finally {
      setBusy(false)
    }
  }
  ...
  const submitLocation = async (event) => {
    ...
    try {
      ...
      setError(err?.data?.message || err?.message || 'Location update failed. Please try again.')
    } finally {
      setBusy(false)
    }
  }
    ...
    <div className="dashboard">
      ...
      <section className="dashboard__grid">
        <article className="panel">
          <h3>Online Status</h3>
          <p className="panel__description">Toggle your availability to receive ride requests</p>
          <div className="details">
            <div>
              <dt>Current Status</dt>
              <dd>
                <strong className={availability ? 'text-success' : 'text-muted'}>
                  {availability ? '🟢 Online - Receiving Rides' : '🔴 Offline - Not Available'}
                </strong>
              </dd>
            </div>
          </div>
        ...
        <article className="panel">
          ...
          <p className="panel__description">Keep your location updated so passengers can find you</p>
      ...
      <section className="panel">
        <div className="panel__header">
          <div>
            ...
            <p className="panel__description">View and manage ride requests from passengers</p>
          </div>
          <button className="secondary" onClick={loadQueue}>Refresh List</button>
        </div>
        {!availability && (
        ...
        {queue.length === 0 && availability && (
          <div className="panel__empty">
            ...
            <p className="text-muted">New ride requests will appear here when passengers book rides.</p>
          </div>
        )}
        {queue.length === 0 && !availability && (
          <div className="panel__empty">
              ...
              {ride.passenger && (
                <div className="details" style={{ marginTop: '0.5rem' }}>
                  <div>
                    ...
                    <dd><strong>{ride.passenger.name}</strong></dd>
                  </div>
                ...
                <div>
                  ...
                  <dd>
                    {ride.pickup_place?.name || ride.pickup_address || 
                     (ride.pickup_lat && ride.pickup_lng ? `${ride.pickup_lat}, ${ride.pickup_lng}` : 'Not specified')}
                ...
                <div>
                  ...
                  <dd>
                    {ride.dropoff_place?.name || ride.dropoff_address || 
                     (ride.drop_lat && ride.drop_lng ? `${ride.drop_lat}, ${ride.drop_lng}` : 'Not specified')}
                ...
                <div>
                  ...
                  <dd>
                    <span className={`status status--${ride.status?.toLowerCase().replace('_', '-')}`}>
                      ...
                      {!['pending', 'accepted', 'picked_up', 'completed'].includes(ride.status) && ride.status}
                    </span>
                  </dd>
                </div>
              </div>
              {ride.notes && (
                <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f8f9fa', borderRadius: '0.5rem' }}>
